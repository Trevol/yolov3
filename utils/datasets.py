...
def defaultImage2Label(imageFile):
    ext = os.path.splitext(imageFile)[-1]
    return imageFile.replace('images', 'labels').replace(ext, '.txt')


class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, pad=0.0, image2label=defaultImage2Label):
        try:
            path = str(Path(path))  # os-agnostic
        ...
        # Define labels
        self.label_files = [image2label(x) for x in self.img_files]
...
def letterbox(img, new_shape=(416, 416), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True):
    ...
    # Scale wh_ratio (new / old)
    r = min(new_shape[0] / shape[0], new_shape[1] / shape[1])
    if not scaleup:  # only scale down, do not scale up (for better test mAP)
        r = min(r, 1.0)

    # Compute padding
    wh_ratio = r, r  # width, height ratios
    ...
    if auto:  # minimum rectangle
        ...
        dw, dh = 0.0, 0.0
        # new_unpad = new_shape
        # wh_ratio = new_shape[0] / shape[1], new_shape[1] / shape[0]  # width, height ratios
        new_unpad = new_shape[::-1]
        wh_ratio = new_shape[1] / shape[1], new_shape[0] / shape[0]
